...
import { Client, ClientStatistics } from '../../modules/clients/types/client.types';
...
export class ClientsStore {
  ...
  readonly activeClients = computed(() => this.clients().filter((client) => client.isActive));

  readonly suspendedClients = computed(() => this.clients().filter((client) => !client.isActive));
   ...
   */
  setClientsWithPagination(clients: Client[], totalElements: number, currentPage: number): void {
    this._clients.set(clients);
   ...
   */
  updateClientById(clientId: number, updates: Partial<Client>): void {
    const currentClients = this._clients();
  ...
  toggleClientStatus(clientId: number): void {
    ...
    if (index !== -1) {
      ...
      newClients[index] = {
        ...
        status: client.isActive ? 'suspended' : 'active',
        updatedAt: new Date(),
      };
      this._clients.set(newClients);
    }
   ...
   */
  setSelectedClient(client: Client, customerToken: string): void {
   ...
   */
  getClientsByStatus(status: 'active' | 'suspended'): Client[] {
  ...
  searchClients(searchTerm: string): Client[] {
    ...
    return this.clients().filter(
      (client) =>
        ...
        client.domainUrl.toLowerCase().includes(term) ||
   ...
   */
  getClientsByDomain(domainPattern: string): Client[] {
    const pattern = domainPattern.toLowerCase();
    return this.clients().filter((client) => client.domainUrl.toLowerCase().includes(pattern));
  }
